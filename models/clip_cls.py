...
class ZSCLIPClassifier(BaseModel):
    ...
    def forward(self, data_dict):
        ...
        # compute logits
        logits = (self.logit_scale * img_feats @ text_feats.T)  # [N, n_cls]

        # map logits back to [B, T, n_cls]
    ...
    @property
    def dtype(self):
        return self.model.logit_scale.dtype
...
class FSCLIPClassifier(ZSCLIPClassifier):
    ...
    def _build_prompts(self, adapter_type):
        ...
        with torch.no_grad():
            text_feats = super().get_text_feats().float()  # [n_classes, C]
    ...
    def get_text_feats(self, class_names=None):
        ...
        # otherwise, we use fixed text features
            ...
            with torch.no_grad():
                text_feats = super().get_text_feats(class_names)
        return self._adjust_dtype(text_feats)

    def get_img_feats(self, imgs):
        # fixed CLIP image features
        with torch.no_grad():
            img_feats = super().get_img_feats(imgs)
